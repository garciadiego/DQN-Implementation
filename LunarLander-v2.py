...
# HYPER_PARAMETERS
hidden_layer1 = 200  #number of hidden layers on 1
hidden_layer2 = 200 #number of hidden layers on 2
hidden_layer3 = 200 #number of hidden layers on 3
#hidden_layer4 = 5 #number of hidden layers on 4
batch_number = 32
gamma = 0.99
...
max_episodes = 1000
memory_size = 200000
learning_rate = 0.00025 #1e-3


#setup enviroment
env = gym.make('LunarLander-v2')
#creates a monitor for the training
env.monitor.start('/tmp/QLearner/LunarLander-v2', force=True)
...
w3 = tf.Variable(tf.random_uniform([hidden_layer2, hidden_layer3], -0.10, 0.10))
b3 = tf.Variable(tf.random_uniform([hidden_layer3], -0.10, 0.10))

#w4 = tf.Variable(tf.random_uniform([hidden_layer3, hidden_layer4], -0.10, 0.10))
#b4 = tf.Variable(tf.random_uniform([hidden_layer4], -0.10, 0.10))

w5 = tf.Variable(tf.random_uniform([hidden_layer3, env.action_space.n], -0.10, 0.10))
b5 = tf.Variable(tf.random_uniform([env.action_space.n], -0.10, 0.10))
...
w3_prime = tf.Variable(tf.random_uniform([hidden_layer2,hidden_layer3],-1.0, 1.0))
b3_prime = tf.Variable(tf.random_uniform([hidden_layer3], -1.0, 1.0))

#w4_prime = tf.Variable(tf.random_uniform([hidden_layer3,hidden_layer4],-1.0, 1.0))
#b4_prime = tf.Variable(tf.random_uniform([hidden_layer4], -1.0, 1.0))

w5_prime = tf.Variable(tf.random_uniform([hidden_layer3, env.action_space.n], -1.0, 1.0))
b5_prime = tf.Variable(tf.random_uniform([env.action_space.n], -1.0, 1.0))
...
#w4_prime_update= w4_prime.assign(w4)
#b4_prime_update= b4_prime.assign(b4)

#w5_prime_update= w5_prime.assign(w5)
#b5_prime_update= b5_prime.assign(b5)

#lists of assigns
Q_update = [w1_prime_update, w2_prime_update, w3_prime_update,
                b1_prime_update, b2_prime_update, b3_prime_update]
#w5_prime_update,b5_prime_update,w4_prime_update,b4_prime_update
...
hidden_activationfx_3 = tf.nn.relu(tf.matmul(hidden_activationfx_2, w3) + b3)
#hidden_activationfx_4 = tf.nn.relu(tf.matmul(hidden_activationfx_3, w4) + b4)

#weights dropout
hidden_activationfx_2 = tf.nn.dropout(hidden_activationfx_2, .5)

#Q-Value
Q = tf.matmul(hidden_activationfx_3, w5) + b5
...
hidden_activationfx_3_prime = tf.nn.relu(tf.matmul(hidden_activationfx_2_prime, w3_prime) + b3_prime)
#hidden_activationfx_4_prime = tf.nn.relu(tf.matmul(hidden_activationfx_3_prime, w4_prime) + b4_prime)
#Prime Layer weights dropout (Saw it in last class)
hidden_activationfx_2_prime = tf.nn.dropout(hidden_activationfx_2_prime, .5)

#Q Prime Value
Q_prime =  tf.matmul(hidden_activationfx_3_prime, w5_prime) + b5_prime
...
#Optimizer
train = tf.train.AdamOptimizer(learning_rate, beta1=0.9, beta2=0.999, epsilon=1e-08).minimize(loss)
